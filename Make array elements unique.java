...
Difficulty: MediumAccuracy: 51.43%Submissions: 49K+Points: 4
...
class Solution {
    public int minIncrements(int[] arr) {
        ...
        // Step 2: Initialize variables for the result and tracking previous element
        int increments = 0;
        int prev = arr[0];
        ...
        for (int i = 1; i < arr.length; i++) {
            ...
            if (arr[i] <= prev) {
                ...
                increments += (prev + 1) - arr[i];
                // Update the previous element
                prev = prev + 1;
            } else {
                // Update the previous element without any increment
                prev = arr[i];
            }
        }
